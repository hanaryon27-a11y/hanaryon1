/* Site-wide effects. Load after supabase.js, at the end of <body>. Not used on overlay/. */

var FX_FLOAT = ['✦', '♪', '✧', '·', '✦', '♫'];   /* floating particle shapes */
var FX_CLICK = '✦';                              /* click / avatar-tap shape. text, emoji, or image URL (data: / https:) */
var FX_COUNT = 14;                               /* number of floating particles */
var FX_TILT  = true;                             /* card tilt on hover */

var FX_LOADER      = true;   /* page transition cover + scale-in */
var FX_LOADER_IMG  = '';     /* loader image URL. empty = auto: mascot(--char) -> favicon -> letter */
var FX_LOADER_TEXT = '';     /* loader caption. empty = nav logo text */
var FX_TRANS_MS    = 800;    /* scale-in duration in ms */
...
(function () {
  ...
  var css = `
    ...
    .fx-tilting{ box-shadow:var(--shadow-hover, 0 16px 36px rgba(15,11,0,.16)); }
    ...
    #fxload .fxload-av{ width:96px; height:96px; border-radius:50%; background:var(--main-light); background-size:cover; background-position:center; display:flex; align-items:center; justify-content:center; font-size:46px; font-weight:800; color:var(--main-dark); box-shadow:0 10px 28px rgba(15,11,0,.14); animation:fxBob 1.1s ease-in-out infinite; }
    #fxload .fxload-av.mascot{ width:150px; height:150px; border-radius:0; background-color:transparent; background-size:contain; background-repeat:no-repeat; box-shadow:none; filter:drop-shadow(0 12px 22px rgba(15,11,0,.16)); }
    @keyframes fxBob{ 0%,100%{ transform:translateY(0) scale(1); } 50%{ transform:translateY(-12px) scale(1.04); } }
    #fxload .fxload-name{ font-weight:800; font-size:15px; letter-spacing:.22em; color:var(--main-dark); }
    #fxload .fxload-dots{ display:flex; gap:7px; }
    #fxload .fxload-dots i{ width:8px; height:8px; background:var(--main); display:block; animation:fxDot 1s ease-in-out infinite; }
  ...
  /* An identity transform left on the wrapper makes it the containing block for
     absolute/fixed children. Clear it when the entrance animation ends. */
  ...
  function revealPage() {
    ...
    setTimeout(function () {
      ...
      if (w) { w.classList.remove('fx-enter'); void w.offsetWidth; w.classList.add('fx-enter'); clearEnter(w); }
  ...
  function build() {
    ...
    if (FX_TILT && mqFine && !mqReduce && !window.__fxTiltOn) {
      ...
      var TILT_SEL = '.card, .item-card, .viewer-card, .notice-item, .up-item, .vod-ph, .song-row, .dg-card';
      var TILT_DEG = 2.5;
      var _tiltEl = null;
  ...
  document.addEventListener('click', function (e) {
    if (e.target.closest('input, textarea, button, a, select, label, .askmodal, .askmask, .modal, .ov, .avatar-wrap, #avatarWrap, .avatar')) return;
